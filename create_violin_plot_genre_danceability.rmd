...
```{r}
create_violin_plot_genre_danceability <- function(genres, sample_size=20, authorization = get_spotify_access_token()) {
    list_df_genres <- genres %>% 
        purrr::map(~ get_genre_track_features(.x, sample_size=sample_size, authorization=authorization) %>% mutate(genre = .x))
...
get_genre_track_features <- function(genre, sample_size=20, authorization = get_spotify_access_token()){
    tracks <- get_genre_tracks(genre, limit=sample_size, authorization = authorization)
